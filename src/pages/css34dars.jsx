            ...
            <Box className='css34dars' pl={{base: '20px' , md: '450px'}} display={'flex'} flexDirection={'column'} gap={'20px'} pt={'40px'} minHeight={{xl:'50vh', md: '42vh'}}>
                <Box width={{base: '100%' , md: '80%'}} gap={'20px'} display={'flex'} flexDirection={'column'} pl={{base: '20px' , md: '100px'}} justifyContent={'center'} alignItems={'flex-start'} bg={'#233242'} height={'500px'} py={{base: '15px', md :'0px'}} rounded={'20px'}>
                    ...
                    <Box width={'90%'} borderLeft={'4px'} pl={'10px'} display={'flex'} flexDirection={'column'} justifyContent={'flex-start'} overflow={'scroll'}  borderColor={'#7FFFD4'} height={'300px'} bg={'#0A1624'} pt={'10px'}>
                     ...
                     <Text fontSize={'20px'} className='purple'>p.uppercase {`{`} <br /> <span style={{paddingLeft: '20px'}} className='red'>text-transform:</span> <span className='yellow'>uppercase;</span> <br /> {`}`}</Text>
                     <Text fontSize={'20px'} className='purple'>p.lowercase {`{`} <br /> <span style={{paddingLeft: '20px'}} className='red'>text-transform:</span> <span className='yellow'>lowercas;</span> <br /> {`}`}</Text>
                     <Text fontSize={'20px'} className='purple'>p.capitalize {`{`} <br /> <span style={{paddingLeft: '20px'}} className='red'>text-transform:</span> <span className='yellow'>capitalize;</span> <br /> {`}`}</Text>
                     
                     
                        
                    </Box>
                    <CopyToClipboard text={`p.uppercase { text-transform: uppercase; }p.lowercase {text-transform: lowercase; }p.capitalize {text-transform: capitalize; }` } onCopy={handleClip} >
                            ...
                            <Table border={'1px'} variant='simple' width={'90%'}>
                                ...
                                <Tbody>
                                    <Tr>
                                        ...
                                        <Td>Matnning bosh harflanishini nazorat qiladi</Td>
                                    </Tr>
                                </Tbody>
